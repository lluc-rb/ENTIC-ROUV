EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 2 3
Title "Data aquisition and transmission circuit"
Date "2020-12-29"
Rev "1"
Comp "RBSM"
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Device:D D1
U 1 1 5FF9A60C
P 4250 2400
F 0 "D1" V 4250 2320 50  0000 R CNN
F 1 "1N4007" V 4205 2320 50  0001 R CNN
F 2 "" H 4250 2400 50  0001 C CNN
F 3 "~" H 4250 2400 50  0001 C CNN
	1    4250 2400
	0    -1   -1   0   
$EndComp
Wire Wire Line
	2550 3750 2050 3750
Wire Wire Line
	2050 3350 2550 3350
Wire Wire Line
	2050 3750 2050 3650
Wire Wire Line
	2050 3650 1950 3650
$Comp
L entic:MPX2100AP Upress1
U 1 1 5FF58EB9
P 1550 3300
F 0 "Upress1" H 1120 3096 50  0001 R CNN
F 1 "MPX2100AP" H 1450 3300 50  0000 R CNN
F 2 "" H 1550 3300 50  0001 C CNN
F 3 "" H 1550 3300 50  0001 C CNN
	1    1550 3300
	1    0    0    -1  
$EndComp
Wire Wire Line
	1950 3450 2050 3450
Wire Wire Line
	2050 3450 2050 3350
$Comp
L power:GND #PWR0101
U 1 1 5FF7138A
P 1550 3850
F 0 "#PWR0101" H 1550 3600 50  0001 C CNN
F 1 "GND" H 1555 3677 50  0000 C CNN
F 2 "" H 1550 3850 50  0001 C CNN
F 3 "" H 1550 3850 50  0001 C CNN
	1    1550 3850
	1    0    0    -1  
$EndComp
Wire Wire Line
	2550 3650 2500 3650
Wire Wire Line
	2150 3450 2550 3450
$Comp
L Device:R Rg1
U 1 1 5FF787EA
P 2350 3650
F 0 "Rg1" V 2250 3650 50  0000 C CNN
F 1 "560" V 2350 3650 50  0000 C CNN
F 2 "" V 2280 3650 50  0001 C CNN
F 3 "~" H 2350 3650 50  0001 C CNN
	1    2350 3650
	0    1    1    0   
$EndComp
Wire Wire Line
	2150 3650 2150 3450
Wire Wire Line
	2200 3650 2150 3650
Connection ~ 2850 3850
Wire Wire Line
	2850 3850 2950 3850
$Comp
L power:GND #PWR0103
U 1 1 5FF2EC06
P 2850 3850
F 0 "#PWR0103" H 2850 3600 50  0001 C CNN
F 1 "GND" H 2855 3677 50  0000 C CNN
F 2 "" H 2850 3850 50  0001 C CNN
F 3 "" H 2850 3850 50  0001 C CNN
	1    2850 3850
	1    0    0    -1  
$EndComp
NoConn ~ 3650 3650
NoConn ~ 3650 3750
NoConn ~ 3650 3850
NoConn ~ 3650 3950
NoConn ~ 3650 4050
NoConn ~ 3650 4250
NoConn ~ 3650 4350
NoConn ~ 4650 4250
NoConn ~ 4650 4150
NoConn ~ 4650 3450
NoConn ~ 4650 3150
NoConn ~ 3650 3350
NoConn ~ 3650 3150
NoConn ~ 3650 2950
Wire Wire Line
	3650 3550 3250 3550
$Comp
L Amplifier_Instrumentation:INA128 Uina126
U 1 1 5FF287BB
P 2850 3550
F 0 "Uina126" H 3294 3596 50  0001 L CNN
F 1 "INA126" H 2950 3800 50  0000 L CNN
F 2 "" H 2950 3550 50  0001 C CNN
F 3 "http://www.ti.com/lit/ds/symlink/ina128.pdf" H 2950 3550 50  0001 C CNN
	1    2850 3550
	1    0    0    -1  
$EndComp
Wire Wire Line
	4650 3050 6500 3050
Wire Wire Line
	4650 2950 6600 2950
Wire Wire Line
	4050 4700 4050 4650
Wire Wire Line
	4150 4700 4050 4700
Connection ~ 4150 4700
Wire Wire Line
	4250 4700 4150 4700
Wire Wire Line
	4250 4650 4250 4700
Wire Wire Line
	4150 4650 4150 4700
NoConn ~ 4050 2550
Wire Wire Line
	6600 2950 6600 4100
$Comp
L Jumper:Jumper_2_Bridged JP1
U 1 1 5FEF614B
P 6900 4100
F 0 "JP1" H 6900 4295 50  0001 C CNN
F 1 "Jumper" H 6900 4203 50  0000 C CNN
F 2 "" H 6900 4100 50  0001 C CNN
F 3 "~" H 6900 4100 50  0001 C CNN
	1    6900 4100
	1    0    0    -1  
$EndComp
Wire Wire Line
	6500 3050 6500 3700
$Comp
L power:GND #PWR0106
U 1 1 5FF5AC71
P 1650 6100
F 0 "#PWR0106" H 1650 5850 50  0001 C CNN
F 1 "GND" H 1655 5927 50  0000 C CNN
F 2 "" H 1650 6100 50  0001 C CNN
F 3 "" H 1650 6100 50  0001 C CNN
	1    1650 6100
	1    0    0    -1  
$EndComp
$Comp
L Device:C Cbypass1
U 1 1 5FF44E70
P 1650 5950
F 0 "Cbypass1" H 1700 6050 50  0000 L CNN
F 1 "0.1uF" H 1700 5850 50  0000 L CNN
F 2 "" H 1688 5800 50  0001 C CNN
F 3 "~" H 1650 5950 50  0001 C CNN
	1    1650 5950
	1    0    0    -1  
$EndComp
Text Notes 1800 4250 0    50   ~ 0
Pressure Sensor Module
Wire Notes Line
	950  4150 3350 4150
Wire Notes Line
	3350 4150 3350 2900
Wire Notes Line
	3350 2900 950  2900
Wire Notes Line
	950  2900 950  4150
Text HLabel 10650 4400 2    50   Output ~ 0
TxD
Text HLabel 10650 4550 2    50   Input ~ 0
RxD
Text HLabel 10650 4250 2    50   Input ~ 0
Vin
Wire Wire Line
	4250 2250 4250 2050
Text HLabel 10650 4700 2    50   Input ~ 0
GND
Wire Wire Line
	9700 4550 9700 4100
Wire Wire Line
	9800 3700 9800 4400
Wire Wire Line
	10000 4700 10000 4900
$Comp
L power:GND #PWR0108
U 1 1 5FEB0C75
P 10000 4900
F 0 "#PWR0108" H 10000 4650 50  0001 C CNN
F 1 "GND" H 10005 4727 50  0000 C CNN
F 2 "" H 10000 4900 50  0001 C CNN
F 3 "" H 10000 4900 50  0001 C CNN
	1    10000 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	9900 4250 9900 2050
Wire Wire Line
	6700 4100 6600 4100
Wire Wire Line
	7100 4100 7200 4100
Wire Wire Line
	7200 3700 6500 3700
Wire Wire Line
	8000 4900 8000 4800
Wire Wire Line
	9700 4100 8800 4100
Wire Wire Line
	9300 3200 9300 4600
Wire Wire Line
	9050 3500 9050 4600
$Comp
L power:GND #PWR0109
U 1 1 5FF73258
P 9300 4900
F 0 "#PWR0109" H 9300 4650 50  0001 C CNN
F 1 "GND" H 9305 4727 50  0000 C CNN
F 2 "" H 9300 4900 50  0001 C CNN
F 3 "" H 9300 4900 50  0001 C CNN
	1    9300 4900
	1    0    0    -1  
$EndComp
$Comp
L Device:C C3
U 1 1 5FF5F4D8
P 9300 4750
F 0 "C3" H 9400 4850 50  0000 L CNN
F 1 "0.33uF" H 9400 4650 50  0000 L CNN
F 2 "" H 9338 4600 50  0001 C CNN
F 3 "~" H 9300 4750 50  0001 C CNN
	1    9300 4750
	1    0    0    -1  
$EndComp
$Comp
L Device:C C4
U 1 1 5FF5ECE3
P 9050 4750
F 0 "C4" H 8900 4850 50  0000 L CNN
F 1 "0.33uF" H 8750 4650 50  0000 L CNN
F 2 "" H 9088 4600 50  0001 C CNN
F 3 "~" H 9050 4750 50  0001 C CNN
	1    9050 4750
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0110
U 1 1 5FF72903
P 9050 4900
F 0 "#PWR0110" H 9050 4650 50  0001 C CNN
F 1 "GND" H 9055 4727 50  0000 C CNN
F 2 "" H 9050 4900 50  0001 C CNN
F 3 "" H 9050 4900 50  0001 C CNN
	1    9050 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	8800 3700 9800 3700
$Comp
L Interface_UART:MAX3232 U1
U 1 1 5FE9E55E
P 8000 3600
F 0 "U1" H 8000 4889 50  0001 C CNN
F 1 "MAX3232" V 8000 4000 98  0000 C CNN
F 2 "" H 8050 2550 50  0001 L CNN
F 3 "https://datasheets.maximintegrated.com/en/ds/MAX3222-MAX3241.pdf" H 8000 3700 50  0001 C CNN
	1    8000 3600
	1    0    0    -1  
$EndComp
NoConn ~ 8800 4300
NoConn ~ 8800 3900
NoConn ~ 7200 3900
NoConn ~ 7200 4300
$Comp
L Device:C C2
U 1 1 5FF45922
P 8800 2850
F 0 "C2" H 8900 2950 50  0000 L CNN
F 1 "0.33uF" H 8900 2750 50  0000 L CNN
F 2 "" H 8838 2700 50  0001 C CNN
F 3 "~" H 8800 2850 50  0001 C CNN
	1    8800 2850
	1    0    0    -1  
$EndComp
$Comp
L Device:C C1
U 1 1 5FF4A2DD
P 7200 2850
F 0 "C1" H 7000 2950 50  0000 L CNN
F 1 "47nF" H 7000 2750 50  0000 L CNN
F 2 "" H 7238 2700 50  0001 C CNN
F 3 "~" H 7200 2850 50  0001 C CNN
	1    7200 2850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0111
U 1 1 5FF5CEB1
P 8000 4900
F 0 "#PWR0111" H 8000 4650 50  0001 C CNN
F 1 "GND" H 8005 4727 50  0000 C CNN
F 2 "" H 8000 4900 50  0001 C CNN
F 3 "" H 8000 4900 50  0001 C CNN
	1    8000 4900
	1    0    0    -1  
$EndComp
Wire Wire Line
	8800 3500 9050 3500
Wire Wire Line
	8800 3200 9300 3200
Wire Wire Line
	9900 2050 4250 2050
Wire Wire Line
	9900 4250 10650 4250
Wire Wire Line
	9800 4400 10650 4400
Wire Wire Line
	9700 4550 10650 4550
Wire Wire Line
	10000 4700 10650 4700
$Comp
L MCU_Module:Arduino_UNO_R3 A1
U 1 1 5FE9FD20
P 4150 3550
F 0 "A1" H 4150 4731 50  0001 C CNN
F 1 "Arduino_UNO" V 4150 3550 98  0000 C CNN
F 2 "Module:Arduino_UNO_R3" H 4150 3550 50  0001 C CIN
F 3 "https://www.arduino.cc/en/Main/arduinoBoardUno" H 4150 3550 50  0001 C CNN
	1    4150 3550
	-1   0    0    -1  
$EndComp
Wire Wire Line
	4650 3550 4850 3550
Wire Wire Line
	4650 3650 4850 3650
Wire Wire Line
	4650 3750 4850 3750
Wire Wire Line
	4650 3850 4850 3850
Wire Wire Line
	4650 3950 4850 3950
Wire Wire Line
	4650 4050 4850 4050
Text Label 4750 3550 0    50   ~ 0
t1
Text Label 4750 3650 0    50   ~ 0
e1
Text Label 4750 3750 0    50   ~ 0
t2
Text Label 4750 3850 0    50   ~ 0
e2
Text Label 4750 4050 0    50   ~ 0
e3
Text Label 4750 3950 0    50   ~ 0
t3
Entry Wire Line
	4850 4050 4950 4150
Entry Wire Line
	4850 3950 4950 4050
Entry Wire Line
	4850 3850 4950 3950
Entry Wire Line
	4850 3750 4950 3850
Entry Wire Line
	4850 3650 4950 3750
Entry Wire Line
	4850 3550 4950 3650
Text Label 5150 5450 2    50   ~ 0
t1
Text Label 5150 5550 2    50   ~ 0
e1
Text Label 5150 6000 2    50   ~ 0
t2
Wire Wire Line
	5050 5550 5550 5550
Wire Wire Line
	5050 5450 5550 5450
Wire Wire Line
	5050 6100 5550 6100
Wire Wire Line
	5050 6000 5550 6000
Wire Wire Line
	5050 6650 5550 6650
Wire Wire Line
	5050 6550 5550 6550
Text Label 5150 6100 2    50   ~ 0
e2
Text Label 5150 6550 2    50   ~ 0
t3
Text Label 5150 6650 2    50   ~ 0
e3
Entry Wire Line
	5050 6000 4950 5900
Entry Wire Line
	5050 6100 4950 6000
Entry Wire Line
	5050 5450 4950 5350
Entry Wire Line
	5050 5550 4950 5450
Wire Wire Line
	5550 5350 5450 5350
Wire Wire Line
	5550 5650 5350 5650
Wire Wire Line
	5450 5900 5450 5350
Connection ~ 5450 5900
Wire Wire Line
	5450 5900 5550 5900
Wire Wire Line
	5350 6200 5350 5650
Connection ~ 5350 6200
Wire Wire Line
	5550 6200 5350 6200
Wire Wire Line
	5450 6450 5550 6450
Wire Wire Line
	5350 6750 5550 6750
Entry Wire Line
	5050 6550 4950 6450
Entry Wire Line
	5050 6650 4950 6550
Wire Wire Line
	5450 6450 5450 5900
Wire Wire Line
	5350 6750 5350 6200
$Comp
L entic:HC-SR04 Uultrasound3
U 1 1 5FF67564
P 6000 6600
F 0 "Uultrasound3" H 6250 6400 50  0001 L CNN
F 1 "HC-SR04" H 6200 6600 50  0000 R CNN
F 2 "" H 6000 6600 50  0001 C CNN
F 3 "" H 6000 6600 50  0001 C CNN
	1    6000 6600
	0    1    -1   0   
$EndComp
$Comp
L entic:HC-SR04 Uultrasound2
U 1 1 5FF670A2
P 6000 6050
F 0 "Uultrasound2" H 6250 5850 50  0001 L CNN
F 1 "HC-SR04" H 6200 6050 50  0000 R CNN
F 2 "" H 6000 6050 50  0001 C CNN
F 3 "" H 6000 6050 50  0001 C CNN
	1    6000 6050
	0    1    -1   0   
$EndComp
Text Notes 6150 6500 1    50   ~ 0
Ultrasonic Ranging Module
Text Notes 6150 5050 1    50   ~ 0
GPS module
$Comp
L entic:HC-SR04 Uultrasound1
U 1 1 5FEE849B
P 6000 5500
F 0 "Uultrasound1" H 6250 5300 50  0001 L CNN
F 1 "HC-SR04" H 6200 5500 50  0000 R CNN
F 2 "" H 6000 5500 50  0001 C CNN
F 3 "" H 6000 5500 50  0001 C CNN
	1    6000 5500
	0    1    -1   0   
$EndComp
$Comp
L entic:NEO-6M-module Ugps
U 1 1 5FEF3864
P 6000 4800
F 0 "Ugps" H 6450 4600 50  0001 R CNN
F 1 "NEO-6M-module" H 6350 4800 50  0000 R CNN
F 2 "" H 6000 4800 50  0001 C CNN
F 3 "" H 6000 4800 50  0001 C CNN
	1    6000 4800
	0    1    -1   0   
$EndComp
Wire Wire Line
	5450 4650 5550 4650
Wire Wire Line
	5350 4950 5550 4950
Wire Wire Line
	4150 4700 4150 4750
$Comp
L power:GND #PWR0113
U 1 1 5FF5D3B9
P 4150 4750
F 0 "#PWR0113" H 4150 4500 50  0001 C CNN
F 1 "GND" H 4155 4577 50  0000 C CNN
F 2 "" H 4150 4750 50  0001 C CNN
F 3 "" H 4150 4750 50  0001 C CNN
	1    4150 4750
	1    0    0    -1  
$EndComp
Connection ~ 5350 4950
Wire Wire Line
	5450 6450 5450 6950
Connection ~ 5450 6450
$Comp
L power:GND #PWR0114
U 1 1 6003D10D
P 5450 6950
F 0 "#PWR0114" H 5450 6700 50  0001 C CNN
F 1 "GND" H 5455 6777 50  0000 C CNN
F 2 "" H 5450 6950 50  0001 C CNN
F 3 "" H 5450 6950 50  0001 C CNN
	1    5450 6950
	1    0    0    -1  
$EndComp
Wire Wire Line
	5550 4850 5050 4850
Wire Wire Line
	5050 4750 5550 4750
Entry Wire Line
	5050 4750 4950 4650
Entry Wire Line
	4950 4750 5050 4850
Wire Wire Line
	4650 3250 4850 3250
Wire Wire Line
	4650 3350 4850 3350
Entry Wire Line
	4850 3250 4950 3350
Entry Wire Line
	4850 3350 4950 3450
Text Label 4750 3250 0    50   ~ 0
tx
Text Label 4750 3350 0    50   ~ 0
rx
Text Label 5150 4750 2    50   ~ 0
tx
Text Label 5150 4850 2    50   ~ 0
rx
Wire Notes Line
	5200 5200 5200 6900
Wire Notes Line
	6250 6900 6250 5200
Wire Wire Line
	5450 4650 5450 5350
Connection ~ 5450 5350
Wire Wire Line
	5350 4950 5350 5650
Connection ~ 5350 5650
Wire Notes Line
	5200 6900 6250 6900
Wire Notes Line
	6250 5200 5200 5200
Wire Notes Line
	5200 5150 5200 4400
Wire Notes Line
	5200 4400 6250 4400
Wire Notes Line
	6250 4400 6250 5150
Wire Notes Line
	5200 5150 6250 5150
Wire Wire Line
	5350 4350 5350 4950
Text Notes 2150 6000 0    50   ~ 0
Bypass capacitor\nSource: 5V pin from Arduino_UNO
Text GLabel 3950 2550 1    50   Output ~ 0
+5V
Text GLabel 8000 2400 1    50   Input ~ 0
+5V
Text GLabel 5350 4350 1    50   Input ~ 0
+5V
Text GLabel 1650 5800 1    50   Input ~ 0
+5V
Text GLabel 1550 3250 1    50   Input ~ 0
+5V
Text GLabel 2850 3250 1    50   Input ~ 0
+5V
Wire Bus Line
	4950 3350 4950 6550
$EndSCHEMATC
